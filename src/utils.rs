...
pub fn encode_files(file_path: &str, pic_path: &str, block_size: Option<usize>) -> anyhow::Result<()> {
    let block_size = block_size.unwrap_or(2048);
    let data = std::fs::read(file_path)?;
    let encoder = encoder::Encoder::new(error::ErrorCorrection::Med);
...
pub fn decode_files(pic_path: &str, output_path: &str) -> anyhow::Result<()> {
    ...
    for name in 0..file_count {
        ...
        let decoded_data = decoder::Decoder::decode(&image)?;
